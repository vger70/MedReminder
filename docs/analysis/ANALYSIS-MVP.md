...
> **Historical document.** This is the original Phase 1 / Phase 2
> plan, written before Increment 0 and kept unchanged (formerly
> `docs/ANALYSIS.md`). Code comments that cite `ANALYSIS §1.x` or
> `ANALYSIS §2.x` refer to the section numbers below. The current
> architecture is described in [`../ANALYSIS.md`](../ANALYSIS.md),
> which also lists where the implementation departed from this plan
> (§11).
