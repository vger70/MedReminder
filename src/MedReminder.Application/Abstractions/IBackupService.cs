...
public interface IBackupService
{
    ...
    // Deletes, through the storage abstraction (C.3++,
    // docs/analysis/ANALYSIS-C3PP-CLOUD-PROVIDERS.md §7.5), every
    // archive older than retentionDays whose name matches the C.3+
    // .mrz pattern — a distinct pattern from PruneOldBackupsAsync, so
    // the two targets never cross-prune when they share a folder.
    // Each archive is judged on its own age, so the most recent .mrz
    // of profile A never shields old .mrz files of profile B.
